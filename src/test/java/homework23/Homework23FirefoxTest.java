...
public class Homework23FirefoxTest {
    ...
    @BeforeAll
    static void setUp() {
        firefoxBrowser = BrowserFactory.getDriver("firefox");
        softAssert = new SoftAssertions();
        firefoxBrowser.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
    }
    ...
    void openGaragePageAndAddCarTest() {
        LoginPage loginPage = new LoginPage(firefoxBrowser);
        loginPage.openForFirefox();
        loginPage.clickGuestLogin();
        softAssert.assertThat(loginPage.isGaragePage())
                .as("Failed to navigate to Garage page").isTrue();

        GaragePage garagePage = new GaragePage(firefoxBrowser);
